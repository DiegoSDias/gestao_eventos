...
export function OrganizerEvents() {
  ...
  const fetchEvents = async (page: number, isLoadMore = false) => {
    try {
      ...
      const response = await api.get(`/organizer/events/me?page=${page}`); 
    ...
    <div>
      ...
      {loading ? (
        ...
        <div className="text-center py-16 bg-white rounded-3xl border border-slate-100 shadow-sm">
          ...
          <h3 className="text-lg font-bold text-slate-800">Nenhum evento por aqui</h3>
          <p className="text-slate-500 mt-1 mx-auto text-sm">
            Você ainda não criou nenhum evento no sistema.
          </p>
          <Link to="/create-event" className="inline-block mt-4 text-sm font-bold text-blue-600 hover:underline">
