...
export function TrashedEvents() {
  ...
  const fetchEvents = async (page: number, isLoadMore = false) => {
    try {
      ...
      const response = await api.get(`/organizer/events/trashed?page=${page}`); 
        ...
        <>
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
            {state.data.map((event) => (
              <div key={event.id} className="opacity-75 grayscale hover:grayscale-0 transition-all">
          ...
          {hasMore && (
            <div className="mt-10 flex justify-center">
              <button
                ...
                className="bg-white border-2 border-red-600 text-red-600 hover:bg-red-50 font-bold py-3 px-8 rounded-xl transition-all disabled:opacity-50 flex items-center gap-2 cursor-pointer"
              >
