...
export function ParticipantEvents() {
  ...
  const fetchEvents = async (page: number, isLoadMore = false) => {
    try {
      ...
      const response = await api.get(`/registrations-me?page=${page}`); 
    ...
    <div>
      <div className="mb-6 flex items-center justify-between">
        <span className="bg-blue-100 text-blue-800 text-sm font-semibold px-3 py-1 rounded-full">
          Total de inscrições: {state.total}
        </span>
      </div>
      ...
      {loading ? (
        ...
        <div className="text-center py-16 bg-white rounded-3xl border border-slate-100 shadow-sm">
          ...
          <h3 className="text-lg font-bold text-slate-800">Nenhuma inscrição encontrada</h3>
          <p className="text-slate-500 mt-1 mx-auto text-sm">
            Você ainda não se inscreveu em nenhum evento.
          </p>
          <Link to="/" className="inline-block mt-4 text-sm font-bold text-blue-600 hover:underline">
            Explorar Eventos →
          </Link>
        </div>
      ) : (
